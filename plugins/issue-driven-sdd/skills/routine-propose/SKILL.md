...
書き切り、1 行目を `未確定の判断: 0 件 — レビューをお願いします` にして `[propose, ai-assess:requested]` を書く（`question` が落ち、AI 評価が起動する）。assess が無いプロジェクトでは `[propose]` だけを書く（`worker.md`「assess があるかを確かめる」）。
...
1 行目は `未確定の判断: 0 件 — レビューをお願いします`、ラベルは `[propose]` → `[propose, ai-assess:requested]` の 2 回書き（assess が無いプロジェクトでは `[propose]` の 1 回）。
