...
4. **デルタ仕様の同期状態を評価する**
   ...
   **同期方針:**
   - デルタ仕様が存在する場合は、同期内容の要約を表示したうえで、ユーザーへの確認なしに必ず同期する。
   - `openspec-sync-specs` スキルの手順でメイン仕様へ適用してから archive を進める。同期をスキップする選択肢は設けない。
...
- 何が起きたかの明確な要約を表示する
- デルタ仕様が存在する場合は、ユーザーに確認せず必ず `openspec-sync-specs` の手順で同期する
