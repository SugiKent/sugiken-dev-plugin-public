...
spec ごとに実装が完了したら、後述するスキルごとに SubAgent にコードレビューをさせ実装したコードを更にブラッシュアップする。
複数の changes spec を作業対象に指示された場合も spec ごとに SubAgent を起動してコードレビューと修正を行う。

コードレビューを行う際は `35-architect-code-review`、 `35-architect-doubt-driven-development` スキルを参照し、正確性・可読性・アーキテクチャ・セキュリティ・パフォーマンス等の多軸でレビューすること。アーキテクチャ／設計構造の是非（変更容易性・SOLID・KISS/YAGNI・DRY・過剰設計の見極め）を深く問う際は `35-architect-principle` スキルの判断基準を参照すること。
